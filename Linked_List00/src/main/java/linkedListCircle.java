 ...
 */
//单向循环链表
public class linkedListCircle<E> extends abstractList<E>{
    ...
    public void add(int index, E element) {
        ...
        if(index == 0){
            ...
            Node<E> last = (size==0)?newFirst:nodeIndex(size-1);
            last.next = newFirst;
            first = newFirst;
        }else {
    ...
    public E remove(int index) {
        ...
        if(index == 0){
            if(size==1){
                ...
                Node<E> last = nodeIndex(size - 1); //这句一定放在first移动之前,否则放在后面的话用nodeIndex会出现越界错误
                first = first.next;
                last.next = first;
            }
        }else {
